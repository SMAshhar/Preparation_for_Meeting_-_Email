...
class MeetingPreparationTasks():
	...
	def summary_and_briefing_task(self, agent, context, objective):
		return Task(
			...
			agent=agent,
			output_file=dedent("briefing_document.md")
		)

	def marketing_email_writer_task(self, agent, participants, context):
		return Task(
			description=dedent(f"""\
				Draft a professional and effective email to promote our product
				to the participants of the meeting. The email should be tailored to the
				interests and needs of the participants, highlighting how our product can
				add value in the context of the meeting.
